...
from training.contract_v4 import (
    ABILITY_VALUES,
    ALLOWED_TYPES,
    CHECK_TYPES,
    MODE_ALLOWED_TYPES,
...
def validate_director(row: dict) -> None:
    ...
    if mode == 'PLAYER_ACTION' and not str(row.get('player_action', '')).strip():
    ...
    if mode == 'CHECK_RESULT':
        ...
        try:
            ...
            raise ValueError(f'{rid}: invalid CHECK_RESULT numbers') from exc
    ...
    if action_type == 'CHECK':
        try:
            ...
            raise ValueError(f'{rid}: invalid CHECK DC') from exc
...
def validate_narrative(row: dict) -> None:
    ...
    if any(x in answer for x in ('<|tool_call>', 'director_action{', '```json', '"type":')):
...
def materialize(row: dict) -> dict:
    if row['kind'] == 'DIRECTOR':
        return {
            ...
            'mode': row['mode'],
            'expected': row['target'],
            'prompt_version': 'v4-compact',
            'messages': to_messages(row),
        }
    messages = [dict(m) for m in row['messages']]
    messages[0]['content'] = NARRATOR_SYSTEM_PROMPT
    return {
        'id': row['id'],
        'kind': 'NARRATIVE',
        'family': row['family'],
...
def main() -> None:
    ...
    for path in pack_paths:
        ...
        rows.extend(current)
        pack_counts[str(path)] = len(current)
    ...
    for row in rows:
        ...
        validate_director(row) if kind == 'DIRECTOR' else validate_narrative(row)

    # Materialize with the compact prompt, then remove exact duplicate inputs.
    # Narrative generators may provide several equally valid answers for one prompt;
    # SFT should see one deterministic target instead of contradictory duplicates.
    materialized_rows: list[tuple[str, dict]] = []
    seen_prompts: dict[tuple, tuple[str, dict]] = {}
    duplicate_inputs_dropped = 0
    for raw in sorted(rows, key=lambda item: str(item['id'])):
        item = materialize(raw)
        prompt_key = (
            item['kind'],
            item['mode'],
            tuple(str(message.get('content', '')) for message in item['messages'][:-1]),
        )
        previous = seen_prompts.get(prompt_key)
        if previous is not None:
            previous_split, previous_item = previous
            if previous_split != raw['split']:
                raise ValueError(
                    f"exact prompt leakage across train/eval: {previous_item['id']} vs {item['id']}"
                )
            if item['kind'] == 'DIRECTOR' and item['expected'] != previous_item['expected']:
                raise ValueError(
                    f"conflicting Director targets for exact prompt: {previous_item['id']} vs {item['id']}"
                )
            duplicate_inputs_dropped += 1
            continue
        seen_prompts[prompt_key] = (raw['split'], item)
        materialized_rows.append((raw['split'], item))

    train = [item for split, item in materialized_rows if split == 'train']
    director_eval = [item for split, item in materialized_rows if split == 'eval' and item['kind'] == 'DIRECTOR']
    narrative_eval = [item for split, item in materialized_rows if split == 'eval' and item['kind'] == 'NARRATIVE']
    ...
    manifest = {
        ...
        'duplicate_inputs_dropped': duplicate_inputs_dropped,
    }
    MANIFEST.parent.mkdir(parents=True, exist_ok=True)
    MANIFEST.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding='utf-8')

    counts = collections.Counter()
    for split, item in materialized_rows:
        if item['kind'] == 'DIRECTOR':
            counts[(split, item['mode'], item['expected']['type'])] += 1
        else:
            counts[(split, 'NARRATIVE', 'TEXT')] += 1
    print(f'v4 compact dataset: raw={len(rows)}; materialized={len(materialized_rows)}; dropped_exact_inputs={duplicate_inputs_dropped}')
    print(f'train={len(train)}; director_eval={len(director_eval)}; narrative_eval={len(narrative_eval)}')
    print(f'Families: {len(family_splits)} (zero leakage)')
    for (split, mode, action), count in sorted(counts.items()):
        print(f'  {split:5s} {mode:13s} {action:16s} {count:5d}')
